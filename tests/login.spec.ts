...
test.describe("Login Tests", () => {
    ...
    test("Dang nhap khong thanh cong voi email khong hop le", async ({
        page,
    }) => {
        const loginPage = new LoginPage(page)
        ...
        await page.waitForTimeout(5000)
        const result = await loginPage.errorMessage
        expect(result).toBeFalsy()
    })

    test("Dang nhap khong thanh cong voi mat khau khong hop le", async ({
        page,
    }) => {
        const loginPage = new LoginPage(page)

        await loginPage.goto()
        await loginPage.login("buingochuyen01@gmail.com", "buingochuyen02")
